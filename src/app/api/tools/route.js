...
import {
  calculatePagination,
  pick,
  toolSearchableFields,
  toolsFilterableFields,
} from "../helpers";
...
export async function GET(req, res) {
  ...
  const { searchTerm, pricing, status, toolTags, ...filterData } = filters;
  const andConditions = [];
  if (searchTerm) {
    andConditions.push({
...
export async function POST(req, res) {
  ...
  try {
    ...
    return NextResponse.json(
      { message: "successfully create ai tool", result },
      { status: 200 }
    );
  } catch (error) {
    return NextResponse.json(
      { message: "filed", error: error.message },
      { status: 500 }
    );
  }
}
